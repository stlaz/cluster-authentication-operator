...
func TestCustomRouterCerts(t *testing.T) {
	...
	routerCertsSecret, err := kubeClient.CoreV1().Secrets("openshift-config-managed").Get(context.Background(), "router-certs", metav1.GetOptions{})
	require.NoError(t, err)
	require.Contains(t, string(routerCertsSecret.Data[ingressConfig.Spec.Domain]), distributedServerCertPem, "expected:\n%s\n\n to contain:\n%s", routerCertsSecret.Data[ingressConfig.Spec.Domain], distributedServerCertPem)
...
func waitForDistributedCert(t *testing.T, kubeClient kubernetes.Interface, expectedCertPem []byte) {
	...
	err := wait.PollImmediate(5*time.Second, 2*time.Minute, func() (bool, error) {
		...
		return currentCert == string(expectedCertPem), nil
	})

	require.NoError(t, err, "failed to wait for the distributed cert, current certificate is %s", currentCert)
}
